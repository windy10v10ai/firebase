...
- **title 上限 4096，随天数增长的字段不下发**：`/proxy/game-start` 不带每日任务的 `history`（30 天可到 17000 字符），直连 `/game/start` 不受限照常带。超限时 `buildProxySuccessHtml` 记 warn 日志（`[Proxy] title too long`）
