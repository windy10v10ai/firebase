...
| `PUT /player/:id/game-preset` | 游戏选项保存 |
...
只对本地 key 生效：单笔上限 50，每日累计上限 2000（UTC 日切）。官方来源不受影响。

超过每日上限时不扣分，但照常返回 200 与当前玩家信息：客户端不管成功失败都会刷新玩家数据，返回 400 只会换来一次无意义的重试和一条错误日志。单笔超限仍返回 400——正常玩法碰不到，只可能来自伪造或客户端 bug。

限额逻辑放 `LocalHostService`，沿用它现有的两段式写法（先只读检查，成功后再写回计数）：
...
「检查」和「写回计数」是两次分开的数据库操作，中间有空隙：同一个 steamId 的两个请求同时进来，可能都读到「今天已用 1980」，各自判断没超 2000 就都放行，实际扣掉 2030。超出的量最多是一笔的上限，也就是 50 积分。要彻底堵住得把读写包进 Firestore 事务，为了 50 积分不值得，本设计不做。

当日累计复用 `LocalRateLimit` 里与本地结算共用的日期字段，见「数据模型」。
...
- 本地限额拒绝（会员积分单笔超限 / 下单超限）：steamId、原因
- 本地会员积分达每日上限跳过扣分：steamId、数量（info 级，属正常流程）

键位、游戏选项、玩家信息、排行榜、查单不记日志——只读或纯个人设置，记了也查不出冒用。下单成功已有日志，不重复添加。

## 数据模型

复用 `LocalRateLimit`（文档 id 为 steamId）。三个当日计数共用一个日期字段，不各自带一个：
...
不做数据迁移。上线后旧文档的 `dailyPointsDate` / `dailyPointsTotal` 不再被读取，等同于当日计数从零开始——相当于上线当天所有人的额度重置一次，可接受。

三个计数在同一个文档里做读-改-写，并发下（同一 steamId 同时结算和消耗积分）可能丢掉一次累加，结果偏向少计，不会多扣玩家。现有代码本来就是这个模式，共用文档只是把范围从一个计数扩到三个。
...
- 共用日期：跨日后三个计数一起归零，不会把昨天的结算积分算进今天

E2E：

- 新增放行名单测试：本地 key 打 7 条放行接口不返回 401；打加点 / 洗点 / 觉醒 / 随机觉醒 / 点赞举报五条必须 401
- 会员积分限额：本地 key 单笔 51 拒绝、累计到 2000 后不再扣分但返回 201、官方 key 不受限
