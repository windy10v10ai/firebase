...
「检查」和「写回计数」是两次分开的数据库操作，中间有空隙：同一个 steamId 的两个请求同时进来，可能都读到「今天已用 980」，各自判断没超 1000 就都放行，实际扣掉 1030。超出的量最多是一笔的上限，也就是 50 积分。要彻底堵住得把读写包进 Firestore 事务，为了 50 积分不值得，本设计不做。
