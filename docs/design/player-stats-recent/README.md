...
- **战斗组独占两份宽度并在组内分两列**，三组高度才齐平、右侧不留空，下方整行留给出装
- **出装一行画在展开层最下面，照游戏结算界面画**：六格物品、两格中立、三格抽选技能，近黑细边不圆角。空物品槽用 Dota 的空槽底图，一眼看得出这局出没出满；中立槽是方格，圆槽是局内 HUD 的样子，结算界面里没有；技能没有空槽底图，画成凹陷。配方共用 Dota 的通用配方图，名字照 Dota 写成「图纸（黑皇杖）」。有东西却没取到图的画「?」，与空槽区分开。悬停或点按图标显示名字，走同一个说明弹层。旧场次没有出装，整行不出现
