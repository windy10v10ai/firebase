...
|---|---|---|---|
| 加字段前 | 713 | 951 | 1092 |
| 加字段后（典型物品技能名） | 1006 | 1342 | 1483 |
| 加字段后（全取最长名） | 1037 | 1383 | 1524 |
...
现网已经证明 1092 能过。涨到 1524 是没验证过的区间，约占 2 KB 的四分之三。
...
真超了再降级，不提前优化：按路由裁剪掉代发报文里的物品技能（做法同 `PROXY_GAME_START_INCLUDE`），或者把它们拆成第二条请求。
