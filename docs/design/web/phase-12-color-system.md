...
> 状态：设计中。功能色方向已定（觉醒色调待选）；网站主色在五个方案里待选，推荐电光蓝。按钮、登录等待、头部退出见 [phase-12-controls.md](phase-12-controls.md)。
...
| **电光蓝（推荐）** | `#2563eb` 纯色加底部光晕、白字 | 5.2 : 1 | 支付宝蓝、Ko-fi 蓝，品牌色只在标识上，不同处出现 | 亮、有网站自己的质感，和游戏渐变一眼分开 |
...
**推荐电光蓝。** 蓝是撞色最少的色相：会撞的只有 Ko-fi 与支付宝，而品牌色只上品牌标识（第 5 节），不会和按钮、标记同处出现。青和绿各自要挤掉一个已有含义。
...
| 英雄觉醒 | `#dd53e7` | 325° | 5.9 : 1 | 方向已定，色调待选 |
| 每日任务 | `#04bbc3` | 200° | 8.1 : 1 | 建议 |
| 技能与物品 | `#e28d4f` | 55° | 7.4 : 1 | 建议 |
...
**扩展顺序**：新功能先取建议色，用完再取预留 1、预留 2。预留 1 离成功绿 21°、预留 2 离交互蓝 19°，排在最后；六个都用完后，再加的功能用中性图标，不再往色环里挤。
...
- phase-7-visual-style.md：第 2 节 accent、链接、赞助平台，第 5 节头部悬停与菜单图标，改成指向本文
- `docs/web/README.md` 第 4 节：补「配色分工」的长期结论（第 1 节表格与三条规则：品牌色只上标识、功能色只上图标、紫金只表示积分）
- `web/CLAUDE.md`：写成规约——调用点不写色值；新功能取色按第 4 节顺序；新品牌按第 5 节登记
