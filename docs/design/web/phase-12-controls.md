...
> 状态：设计中。头部退出暂定方案 H2；登录等待推荐 L1；主按钮颜色随 [phase-12-color-system.md](phase-12-color-system.md) 的网站主色待选（推荐电光蓝）。依赖批次 10 PR 2 与批次 11，在它们之后实现。
...
**激活页提交不再用整页遮罩**，改成提交按钮自己的「处理中」状态。全屏 `Spinner` 两个使用方都换掉后删除。
...
- **等待超过 15 秒**：卡片底部出现次按钮「再试一次」，不打断正在进行的请求
- **失败**：卡片里一块等宽小字，写明卡在第几步、错误码、错误细分、尝试次数、总耗时、发生时间（带时区）、玩家 ID，并提示反馈问题时截图带上这一块。发生时间与 ID 用来在生产日志里找到同一次登录
...
- phase-7-visual-style.md 第 4 节按钮：改成指向本文
