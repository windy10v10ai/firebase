...
- **立绘、英雄名、技能图标不放骨架块。** 它们是 `config/awaken.ts` 里的常量，首帧就能画对，等接口回来只会让 39 张卡白等一轮。加载中换成骨架块的只有卡底那条按钮——「觉醒 / 已觉醒」是接口才知道的值
- **限免角标等接口回来才出现。** 它的条件是「限免且未觉醒」，加载中一律显示的话，已觉醒的英雄会先挂一个角标再收回去
...
**浏览器实测用 Playwright 路由拦截代替后端**（固定 `PlayerInfo`，无需登录、不碰生产数据）。还没做、要等条件的：

- **真实登录态下的端到端实测**。需要 Steam 登录，且解锁会真的扣积分，得用测试账号或本地 API + emulator
