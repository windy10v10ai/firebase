...
批次 0、1、2、3ab、2g、7、8a、8b 已完成：浏览器直连 API、Steam 登录、游戏界面风格 token、首页与个人主页均已落地。
...
| 8c 会员页与商业披露 | 待建 | 会员页订阅按钮统一紫色；披露页、数据表与文档收尾 | 进行中 |
...
8a 与 8b 已完成。8c 负责会员页、商业披露与直接相关的共用组件：

- 会员页保留既有排版；爱发电与 Ko-fi 的订阅按钮统一使用紫色主按钮，平台标题仍使用各自品牌色。
- 商业披露使用普通内容卡片与标题色；`DataTable` 在窄屏显示标签—内容卡片列表，平板及更宽屏幕保留表格。
- 移除这两个页面及其直接共用组件中的 Tailwind 原始色，并同步修正与现有实现不符的设计文档和完成状态。
