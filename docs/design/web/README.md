...
四个页面，拆成三个 task，各自一个 PR。三者的文件不重叠，可以并行。

| task | 页面 | 做到哪一步 |
|------|------|------|
| 8a | 首页 | 删掉「关于项目」；标题下方、创意工坊上方加两块：登录引导（描述 + 数据面板示意 + 登录按钮）、主要页面卡片 |
| 8b | 个人主页 | 身份卡四行数值按勇士紫 / 会员金上色，战绩卡拉开层级，宽屏由一列改分栏 |
| 8c | 会员页、商业披露 | 会员页只剩排版，颜色批次 7 已做完；披露页整页跟上设计 token |

共用组件按 task 分：`Card` 归 8a，`DataTable` 与 `Section` 归 8c，8b 只改 `web/app/profile/` 下的文件。

**属性与觉醒的入口不在这一批。** 个人主页的功能入口卡要等 3a、3b 上线才加，否则点进去是空页。
...
| 通道 | 批次 | 碰的代码 |
|------|------|------|
| A 首页 | 8a | `web/app/page.tsx`、`web/app/components/Card.tsx` |
| B 个人主页 | 8b | `web/app/profile/` |
| C 会员页与披露 | 8c | `web/app/membership/`、`web/app/legal/`、`web/app/components/DataTable.tsx` |
| D 外部 | 9 申请 Steam Web API key | 无代码，等审批 |
| E 埋点 | 5 GA4 | `web/app/layout.tsx` 加一个 lib |

各通道的文件不重叠，同时开工不会互相覆盖。多个会话并行时按根目录 [CLAUDE.md](../../../CLAUDE.md) 的约定用 `git worktree` 隔离。

### 阻塞关系

- ~~3ab-api 挡着属性页与觉醒页~~。已完成（#1163）：四个接口都挂上了 `@AllowWeb()`。归属校验不用另写，guard 对带 `:steamId` 的路由自动比对 uid，不一致返回 403。
- ~~视觉风格挡着属性、觉醒与页面布局三批~~。已完成（#1165）：token 取值、字体与基础组件都已就位，取值见 [phase-7-visual-style.md](phase-7-visual-style.md)，后面几批直接用。
...
5. **4 游戏联动**——等前两页上线
6. 页面布局（8a/8b/8c）、昵称头像、GA4、会员剩余**都不挡别人**，随时插队
