...
|------|------|------|------|------|
| `/` | 首页 | 否 | 头部 logo | 已有 |
| `/profile/<steamId>` | 个人主页 | 是 | 头部的 ID 按钮 | 已有 |
| `/profile/<steamId>/property` | 属性加点 | 是 | 头部菜单 | 批次 3a |
| `/profile/<steamId>/awaken` | 英雄觉醒 | 是 | 头部菜单 | 批次 3b |
| `/my/*` | 玩家页面的未登录入口，登录后跳到 `/profile/<自己的 id>/*` | — | 菜单在未登录时指向这里 | 已有 |
| `/wiki/abilities`、`/wiki/items` | 技能与物品（开局抽选一览） | 否 | 头部菜单 | 未排期 |
| `/membership` | 会员介绍与订阅 | 否 | 头部菜单 | 已有 |
| `/regist/afdian`、`/regist/kofi` | 手动激活 | 否 | 从 `/membership` 进 | 已有 |
| `/legal/disclosure` | 商业披露 | 否 | 页脚 | 已有 |
| `/login/callback` | 登录回调 | — | 不进菜单 | 已有 |
...
| 2g 顶部布局 | 待建 | 头部结构重做：站内外分组、两端同一套菜单、控件尺寸统一 | 未开始 |
...
- **顶部布局挡着属性页与觉醒页的入口**，但不挡页面本体。菜单结构先定，那两页上线时才不用回头改头部。
...
1. **3ab-api**——最小、最独立，做完属性页就只剩前端
2. **2g 顶部布局**——与上一条同时做，不在同一个目录
