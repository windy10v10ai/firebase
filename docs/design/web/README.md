...
| 2 Steam 登录 | #1118 | 后端 `auth` 模块、guard 支持 ID Token、`@AllowWeb()`；网站 Firebase SDK、登录 / 回调、`AuthProvider`、头部登录按钮；激活页自动填 ID；「我的」页面（`GET /player/:id/info` 加 `@AllowWeb`） | 登录后激活页自动填 ID，「我的」页面能看到会员和属性 | [phase-2-steam-login.md](phase-2-steam-login.md)；登录入口交互见 [phase-2-login-entry.md](phase-2-login-entry.md) |
