...
- **桌面头部**：左边 logo 回首页；中间 属性、觉醒、会员订阅、创意工坊、GitHub；右边 登录按钮 / `ID 12345678`（点进个人主页）+ 退出、语言切换。属性与觉醒两项随 3a、3b 一起加，wiki 的入口等那一批再加。
- **窄屏**：中间几项收进汉堡菜单，登录 / ID 与语言常驻。10 位 ID 在窄屏放不下，登录按钮与 ID 都只留图标。
