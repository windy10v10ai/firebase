...
| 7 视觉风格 | 待建 | 按游戏界面风格改 token 取值、字体、面板装饰，打磨基础组件 | 未开始 |
...
- **视觉风格挡着三批**（属性、觉醒、页面布局），是唯一的瓶颈。它改 `web/app/globals.css` 的 token 取值与 `web/app/components/ui/` 下的基础组件，所有页面都吃这套。**范围要收住：只改 token 取值和字体，不顺手重构组件**，否则后面三批一直等。
