...
批次 0、1 与批次 2 的大半已上线：浏览器直连 API，antd 已删、Tailwind 4 与设计 token 就位，Steam 登录跑通，登录态能带着 ID Token 调 API。

养成功能（属性、觉醒）仍然只在游戏内，网站还没有对应页面，也还没有登录后才能看的页面——这是接下来几批要做的事。
...
|------|------|------|------|------|
| `/` | 首页 | 否 | 头部 logo | 已有 |
| `/profile` | 个人主页 | 是 | 头部的 ID 按钮 | 批次 2e |
| `/property` | 属性加点 | 是 | 头部菜单 | 批次 3a |
| `/awakening` | 英雄觉醒 | 是 | 头部菜单 | 批次 3b |
...
要登录的三个页面放进同一个路由组 `app/(player)/`，门禁写在组的 layout 里。批次 2e 建好这层之后，3a、3b 把页面文件放进这个目录就自带门禁，不用各写一遍判断。

排行榜暂不排期：接口还没有，也没定要不要上网站。

### 菜单

- **桌面头部**：左边 logo 回首页；中间 属性、觉醒、会员订阅、创意工坊、GitHub；右边 登录按钮 / `ID 12345678`（点进个人主页）+ 退出、语言切换。
- **窄屏**：中间五项收进汉堡菜单，登录 / ID 与语言常驻。
...
- 用户页面客户端渲染，不做 SSR；门禁写在路由组的 layout 里，不是每个页面自己判断。
...
| 2 Steam 登录 | #1118 | 后端 `auth` 模块与 guard、网站登录跳转与回调、门禁路由组、个人主页、激活页自动填 ID | 2a–2d 已完成，剩 2e、2f |
| 7 视觉风格 | 待建 | 按游戏界面风格改 token 取值、字体、面板装饰，打磨基础组件 | 未开始 |
| 3a 属性页 | #1119 | `/property`：查看、加点、洗点 | 未开始 |
| 3b 觉醒页 | #1120 | `/awakening`：已觉醒列表、解锁、随机 | 未开始 |
...
- **视觉风格排在属性、觉醒之前**。这两页 UI 量最大，风格定完再写才不返工。改的是 token 取值和基础组件，已有页面跟着变。
- **属性排在觉醒之前**。游戏内加点已经禁掉，这是玩家最直接的痛点。
- **游戏联动要等 3a、3b 上线**，否则「前往网站」没有落点。
- **页面布局、昵称头像、GA4、会员剩余都不挡别人**，排在后面，随时可以插队。
