...
| 我的、属性、觉醒、会员、记录 | 静态外壳 + 浏览器登录后调 API | 要登录才能看，不需要 SEO |
...
| 3c 会员 | 积分、到期时间、支付宝二维码购买 | 同上 | `phase-3c-membership.md` |
| 3d 记录 | 积分变动与消费记录；后端有无现成记录待确认，可能要新接口 | 同上 | `phase-3d-records.md` |
| 4 收尾 | game 仓库：「前往网站」按钮、刷新按钮、FAQ；网站加 GA4 | 网站操作后回游戏刷新能看到结果 | `phase-4-game-sync.md` |

批次 0、1、2 顺序执行。批次 3 的四项互相独立，建议 3a 先做，它直接对应加点被禁的痛点。
