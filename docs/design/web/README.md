...
|------|------|------|------|------|
| `/` | 首页 | 否 | 头部 logo | 已有 |
| `/profile/<steamId>` | 个人主页 | 是 | 头部的 ID 按钮 | 批次 2e |
| `/profile/<steamId>/property` | 属性加点 | 是 | 头部菜单 | 批次 3a |
| `/profile/<steamId>/awaken` | 英雄觉醒 | 是 | 头部菜单 | 批次 3b |
| `/my/*` | 玩家页面的未登录入口，登录后跳到 `/profile/<自己的 id>/*` | — | 菜单在未登录时指向这里 | 批次 2e |
| `/wiki/abilities`、`/wiki/items` | 技能与物品抽选一览 | 否 | 头部菜单 | 未排期 |
...
- **桌面头部**：左边 logo 回首页；中间 属性、觉醒、会员订阅、创意工坊、GitHub；右边 登录按钮 / `ID 12345678`（点进个人主页）+ 退出、语言切换。wiki 的入口等那一批再加。
- **窄屏**：中间几项收进汉堡菜单，登录 / ID 与语言常驻。
...
| 2 Steam 登录 | #1118 | 后端 `auth` 模块与 guard、网站登录跳转与回调、`/my/*` 门禁、个人主页、激活页自动填 ID | 2a–2d 已完成，剩 2e、2f |
