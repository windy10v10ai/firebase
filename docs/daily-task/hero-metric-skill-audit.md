...
- 建议直接删除或替换：33 条。
- 原需实测的 6 条 `GetStuns()` / `GetHealing()` 口径已全部确认，当前无待定项。
- 建议新增或替换 metric：40 个英雄。
...
- 完整英雄任务表已同步使用新档位：`hero_damage` 整体上调、`damage_taken` 以约 200,000 为中心、`stun_duration` 最低 40。
...
- 大数值 target 优先使用偶数，确保乘以 1.5 后仍为整数；小整数任务使用加法档。
...
| 蝙蝠骑士（`batrider`） | 大招燃烧枷锁已确认计入 stun | **保留 `stun_duration`**；但大招冷却较长，target 取低档 | 40 |
...
### `tower_kills` target 重新分档

`tower_kills` 计算防御塔最后一击，不是单纯的建筑伤害。每局总共只有 11 座防御塔，还会被队友或小兵抢到，因此不应沿用 8～14 的高 target。

| 1★ target | 适用英雄 | 判断依据 |
|---:|---|---|
| 3 | 普通具备推塔能力 | 能推塔，但抢到最后一击的优势有限 |
...
| 朗戈（`largo`） | `stun_duration` | 蛙力千钧的周期眩晕会计入 | 保留，target 50 |
| 朗戈（`largo`） | `healing` | 动人之舐的恢复会计入 | 保留，target 40,000 |
...
| 斧王<br>`axe` | 先手、承伤、控制、核心 | `hero_damage` | 先手、承伤、控制、核心；输出多来自进场、控制或反伤过程，不是纯输出定位，因此保留时取最低档 | 高 | 250,000 | 300,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 术士<br>`warlock` | 辅助、先手、控制 | `hero_damage` | 辅助、先手、控制；主要依靠范围技能和团战参与累计伤害，target 应取低档 | 高 | 500,000 | 320,000 | 保留并调整 target | — | DOTA 7.41 |
| 莱恩<br>`lion` | 辅助、控制、爆发、先手 | `stun_duration` | 裂地尖刺可范围命中多名英雄，配合妖术形成两个可稳定循环的控制技能 | 高 | 110 | 100 | 保留并调整 target | — | DOTA 7.41 |
| 莱恩<br>`lion` | 辅助、控制、爆发、先手 | `hero_damage` | 辅助、控制、爆发、先手；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 暗影萨满<br>`shadow_shaman` | 辅助、推进、控制、爆发、先手 | `tower_kills` | 辅助、推进、控制、爆发、先手；拥有明确推进定位 | 高 | 3 | 5 | 保留并调整 target | — | DOTA 7.41 |
| 暗影萨满<br>`shadow_shaman` | 辅助、推进、控制、爆发、先手 | `hero_damage` | 核心价值是妖术与枷锁的连续控制，且已有推塔任务；控制比输出更能体现英雄定位 | 高 | 500,000 | — | 删除/替换 | `stun_duration`（建议 100，替换 hero_damage） | DOTA 7.41 |
| 斯温<br>`sven` | 核心、控制、先手、承伤、爆发 | `hero_damage` | 核心、控制、先手、承伤、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 750,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
| 斯温<br>`sven` | 核心、控制、先手、承伤、爆发 | `stun_duration` | 风暴之拳（CD 最低约 12 秒） | 高 | 80 | 50 | 保留并调整 target | — | DOTA 7.41 |
| 主宰<br>`juggernaut` | 核心、推进、机动 | `hero_damage` | 核心、推进、机动；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 550,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 钢背兽<br>`bristleback` | 核心、承伤、先手、爆发 | `hero_damage` | 核心、承伤、先手、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 700,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
| 潮汐猎人<br>`tidehunter` | 先手、承伤、控制、爆发、核心 | `damage_taken` | 先手、承伤、控制、爆发、核心；有明确 Durable 定位和前排能力 | 高 | 700,000 | 220,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 龙骑士<br>`dragon_knight` | 核心、推进、承伤、控制、先手、爆发 | `damage_taken` | 核心、推进、承伤、控制、先手、爆发；有明确 Durable 定位和前排能力 | 高 | 650,000 | 220,000 | 保留并调整 target | — | DOTA 7.41 |
| 龙骑士<br>`dragon_knight` | 核心、推进、承伤、控制、先手、爆发 | `stun_duration` | 神龙摆尾（时长最高约 2.4 秒，CD 最低约 10 秒） | 高 | 80 | 60 | 保留并调整 target | — | DOTA 7.41 |
| 龙骑士<br>`dragon_knight` | 核心、推进、承伤、控制、先手、爆发 | `hero_damage` | 当前有三个任务；明确保留 `damage_taken` 220,000 和 `stun_duration` 60，按“每英雄最多两个 metric”删除输出 | 高 | 650,000 | — | 删除/替换 | 保留 `damage_taken` + `stun_duration` | DOTA 7.41 |
| 幻影刺客<br>`phantom_assassin` | 核心、机动 | `hero_damage` | 核心、机动；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 900,000 | 600,000 | 保留并调整 target | — | DOTA 7.41 |
| 幻影刺客<br>`phantom_assassin` | 核心、机动 | `kills` | 核心、机动；具备收割或爆发能力 | 高 | 60 | 50 | 保留并调整 target | — | DOTA 7.41 |
| 影魔<br>`nevermore` | 核心、爆发 | `hero_damage` | 核心、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 600,000 | 550,000 | 保留并调整 target | `kills`（建议 40） | DOTA 7.41 |
| 死灵法师<br>`necrolyte` | 核心、爆发、承伤、控制 | `hero_damage` | 核心、爆发、承伤、控制；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 600,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
| 死灵法师<br>`necrolyte` | 核心、爆发、承伤、控制 | `healing` | 死亡脉冲（治疗/恢复参数最高 130，CD 最低约 5 秒） | 高 | 100,000 | 60,000 | 保留并调整 target | — | DOTA 7.41 |
| 风行者<br>`windrunner` | 核心、辅助、控制、机动、爆发 | `hero_damage` | 核心、辅助、控制、机动、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 700,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 开膛手杰克<br>`brewmaster` | 敏捷核心、爆发、控制、兼具治疗 | `hero_damage` | 敏捷核心、爆发、控制、兼具治疗；自定义技能组以连招、爆发或持续输出为主 | 高 | 700,000 | 500,000 | 保留并调整 target | — | game 自定义技能 |
| 阿尔托莉雅・潘德拉贡<br>`broodmother` | 全属性核心、爆发、正面作战 | `hero_damage` | 全属性核心、爆发、正面作战；自定义技能组以连招、爆发或持续输出为主 | 高 | 700,000 | 500,000 | 保留并调整 target | — | game 自定义技能 |
| 阿尔托莉雅・潘德拉贡<br>`broodmother` | 全属性核心、爆发、正面作战 | `damage_taken` | 自定义阿尔托莉雅为全属性近战核心，具有魔免与正面作战能力，可承担承伤任务 | 高 | 600,000 | 180,000 | 保留并调整 target | — | game 自定义技能 |
| 混沌骑士<br>`chaos_knight` | 核心、控制、承伤、推进、先手 | `stun_duration` | 混乱之箭（CD 最低约 10 秒） | 高 | 90 | 60 | 保留并调整 target | — | DOTA 7.41 |
| 混沌骑士<br>`chaos_knight` | 核心、控制、承伤、推进、先手 | `tower_kills` | 核心、控制、承伤、推进、先手；拥有明确推进定位 | 高 | 3 | 4 | 保留并调整 target | — | DOTA 7.41 |
| 孙悟空<br>`chen` | 全属性核心、持续输出、短眩晕 | `stun_duration` | 界王拳（时长最高约 0.2 秒） | 高 | 90 | 40 | 保留并调整 target | — | game 自定义技能 |
| 孙悟空<br>`chen` | 全属性核心、持续输出、短眩晕 | `hero_damage` | 全属性核心、持续输出、短眩晕；自定义技能组以连招、爆发或持续输出为主 | 高 | 550,000 | 500,000 | 保留并调整 target | — | game 自定义技能 |
| 克林克兹<br>`clinkz` | 核心、机动、推进 | `hero_damage` | 核心、机动、推进；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 克林克兹<br>`clinkz` | 核心、机动、推进 | `tower_kills` | 核心、机动、推进；拥有明确推进定位 | 高 | 3 | 4 | 保留并调整 target | — | DOTA 7.41 |
...
| 发条技师<br>`rattletrap` | 先手、控制、承伤、爆发 | `hero_damage` | 先手、控制、承伤、爆发；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 末日使者<br>`doom_bringer` | 核心、控制、先手、承伤、爆发 | `hero_damage` | 核心、控制、先手、承伤、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
| 卓尔游侠<br>`drow_ranger` | 核心、控制、推进 | `hero_damage` | 核心、控制、推进；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 550,000 | 保留并调整 target | — | DOTA 7.41 |
| 卓尔游侠<br>`drow_ranger` | 核心、控制、推进 | `tower_kills` | 核心、控制、推进；拥有明确推进定位 | 高 | 3 | 3 | 保留 | — | DOTA 7.41 |
| 大地之灵<br>`earth_spirit` | 爆发、机动、控制、先手、承伤 | `stun_duration` | 巨石翻滚（时长最高约 0.9 秒，CD 最低约 4 秒） | 高 | 90 | 60 | 保留并调整 target | — | DOTA 7.41 |
| 大地之灵<br>`earth_spirit` | 爆发、机动、控制、先手、承伤 | `damage_taken` | 爆发、机动、控制、先手、承伤；有明确 Durable 定位和前排能力 | 高 | 600,000 | 200,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 德鲁伊<br>`lone_druid` | 核心、推进、承伤 | `tower_kills` | 核心、推进、承伤；拥有明确推进定位 | 高 | 3 | 5 | 保留并调整 target | — | DOTA 7.41 |
| 露娜<br>`luna` | 核心、爆发、推进 | `stun_duration` | 月光提供约 0.6 秒短眩晕，冷却最低约 6 秒；主要靠高频短控累计 | 高 | 90 | 40 | 保留并调整 target | — | DOTA 7.41 |
| 露娜<br>`luna` | 核心、爆发、推进 | `hero_damage` | 核心、爆发、推进；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 变体精灵<br>`morphling` | 核心、机动、承伤、爆发、控制 | `hero_damage` | 核心、机动、承伤、爆发、控制；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 550,000 | 保留 | — | DOTA 7.41 |
| 齐天大圣<br>`monkey_king` | 核心、机动、控制、先手 | `stun_duration` | 棒击大地提供最高约 1.3 秒直线范围眩晕，冷却最低约 15 秒 | 高 | 90 | 50 | 保留并调整 target | — | DOTA 7.41 |
| 齐天大圣<br>`monkey_king` | 核心、机动、控制、先手 | `hero_damage` | 核心、机动、控制、先手；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 娜迦海妖<br>`naga_siren` | 核心、辅助、推进、控制、先手、机动 | `stun_duration` | 实测确认网计入 stun，大招海妖之歌不计入；网的时长足以独立完成任务 | 高 | 90 | 80 | 保留并调整 target | — | 实测 + DOTA 7.41 |
| 娜迦海妖<br>`naga_siren` | 核心、辅助、推进、控制、先手、机动 | `hero_damage` | 核心、辅助、推进、控制、先手、机动；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 550,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 自然先知<br>`furion` | 核心、推进、机动、爆发 | `stun_duration` | 发芽用树木困人，不是引擎眩晕/缠绕状态 | 高 | 90 | — | 删除/替换 | `tower_kills`（建议 5） | DOTA 7.41 |
| 自然先知<br>`furion` | 核心、推进、机动、爆发 | `hero_damage` | 核心、推进、机动、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | `tower_kills`（建议 5） | DOTA 7.41 |
| 暗夜魔王<br>`night_stalker` | 核心、先手、承伤、控制、爆发 | `healing` | 午夜盛宴只治疗自身，且依赖攻击/夜晚 | 高 | 100,000 | — | 删除/替换 | `damage_taken`（建议 200,000） | DOTA 7.41 |
| 暗夜魔王<br>`night_stalker` | 核心、先手、承伤、控制、爆发 | `hero_damage` | 核心、先手、承伤、控制、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | `damage_taken`（建议 200,000） | DOTA 7.41 |
| 司夜刺客<br>`nyx_assassin` | 控制、爆发、先手、机动 | `stun_duration` | 穿刺提供最高约 2 秒范围眩晕，尖刺外壳可反制多个伤害来源 | 高 | 90 | 100 | 保留并调整 target | — | DOTA 7.41 |
| 司夜刺客<br>`nyx_assassin` | 控制、爆发、先手、机动 | `hero_damage` | 控制、爆发、先手、机动；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 石鳞剑士<br>`pangolier` | 核心、爆发、控制、承伤、机动、先手 | `hero_damage` | 核心、爆发、控制、承伤、机动、先手；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 八云紫<br>`phantom_lancer` | 智力核心、范围爆发、控制 | `hero_damage` | 智力核心、范围爆发、控制；自定义技能组以连招、爆发或持续输出为主 | 高 | 550,000 | 450,000 | 保留并调整 target | — | game 自定义技能 |
| 帕克<br>`puck` | 先手、控制、机动、爆发 | `stun_duration` | 梦境缠绕（时长最高约 0.5 秒，CD 最低约 75 秒） | 高 | 90 | 40 | 保留并调整 target | — | DOTA 7.41 |
| 帕克<br>`puck` | 先手、控制、机动、爆发 | `hero_damage` | 先手、控制、机动、爆发；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | — | DOTA 7.41 |
| 帕格纳<br>`pugna` | 爆发、推进 | `healing` | 生命汲取可反向对友方英雄输送生命，可持续施法累计治疗量 | 高 | 100,000 | 60,000 | 保留并调整 target | — | DOTA 7.41 |
| 帕格纳<br>`pugna` | 爆发、推进 | `hero_damage` | 爆发、推进；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | — | DOTA 7.41 |
| 痛苦女王<br>`queenofpain` | 核心、爆发、机动 | `hero_damage` | 核心、爆发、机动；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 550,000 | 保留 | `kills`（建议 40） | DOTA 7.41 |
| 雷泽<br>`razor` | 核心、承伤、爆发、推进 | `hero_damage` | 核心、承伤、爆发、推进；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 550,000 | 保留 | `damage_taken`（建议 200,000） | DOTA 7.41 |
...
| 裂魂人<br>`spirit_breaker` | 核心、先手、控制、承伤、机动 | `damage_taken` | 核心、先手、控制、承伤、机动；有明确 Durable 定位和前排能力 | 高 | 600,000 | 220,000 | 保留并调整 target | — | DOTA 7.41 |
| 风暴之灵<br>`storm_spirit` | 核心、机动、爆发、先手、控制 | `hero_damage` | 核心、机动、爆发、先手、控制；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 550,000 | 保留 | `kills`（建议 40） | DOTA 7.41 |
| 茶神<br>`techies` | 爆发、控制 | `hero_damage` | 爆发、控制；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | — | DOTA 7.41 |
| 茶神<br>`techies` | 爆发、控制 | `kills` | 爆发、控制；具备收割或爆发能力 | 高 | 46 | 32 | 保留并调整 target | — | DOTA 7.41 |
| 圣堂刺客<br>`templar_assassin` | 核心、机动 | `stun_duration` | 灵能陷阱只有减速，基础技能没有眩晕或缠绕 | 高 | 90 | — | 删除/替换 | `tower_kills`（建议 4） | DOTA 7.41 |
| 圣堂刺客<br>`templar_assassin` | 核心、机动 | `kills` | 核心、机动；具备收割或爆发能力 | 高 | 46 | 40 | 保留并调整 target | `tower_kills`（建议 4） | DOTA 7.41 |
| 恐怖利刃<br>`terrorblade` | 核心、推进、爆发 | `healing` | 狂魔提供自身生命恢复，魂断交换生命百分比；统计口径不可靠 | 高 | 100,000 | — | 删除/替换 | `tower_kills`（建议 5） | DOTA 7.41 |
| 恐怖利刃<br>`terrorblade` | 核心、推进、爆发 | `hero_damage` | 核心、推进、爆发；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 550,000 | 保留 | `tower_kills`（建议 5） | DOTA 7.41 |
| 伐木机<br>`shredder` | 爆发、承伤、机动 | `healing` | 活性护甲提供自身生命恢复，不是主动治疗 | 高 | 100,000 | — | 删除/替换 | `damage_taken`（建议 220,000） | DOTA 7.41 |
| 伐木机<br>`shredder` | 爆发、承伤、机动 | `hero_damage` | 爆发、承伤、机动；可依靠范围或爆发技能稳定累计英雄伤害，target 取中档 | 高 | 550,000 | 400,000 | 保留并调整 target | `damage_taken`（建议 220,000） | DOTA 7.41 |
...
| 不朽尸王<br>`undying` | 辅助、承伤、控制、爆发 | `hero_damage` | 辅助、承伤、控制、爆发；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 熊战士<br>`ursa` | 核心、承伤、控制 | `hero_damage` | 核心、承伤、控制；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | `kills`（建议 40） | DOTA 7.41 |
| 复仇之魂<br>`vengefulspirit` | 辅助、先手、控制、爆发、机动 | `stun_duration` | 魔法箭（CD 最低约 11 秒） | 高 | 90 | 60 | 保留并调整 target | — | DOTA 7.41 |
| 复仇之魂<br>`vengefulspirit` | 辅助、先手、控制、爆发、机动 | `hero_damage` | 辅助、先手、控制、爆发、机动；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 沉渊之剑<br>`visage` | 敏捷核心、暴击、机动收割 | `hero_damage` | 敏捷核心、暴击、机动收割；自定义技能组以连招、爆发或持续输出为主 | 高 | 700,000 | 500,000 | 保留并调整 target | — | game 自定义技能 |
...
| 虚无之灵<br>`void_spirit` | 核心、机动、爆发、控制 | `hero_damage` | 核心、机动、爆发、控制；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | `kills`（建议 40，替换 stun_duration） | DOTA 7.41 |
| 编织者<br>`weaver` | 核心、机动 | `hero_damage` | 核心、机动；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
...
| 寒冬飞龙<br>`winter_wyvern` | 辅助、控制、爆发 | `hero_damage` | 辅助、控制、爆发；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 骷髅王<br>`skeleton_king` | 核心、辅助、承伤、控制、先手 | `stun_duration` | 冥火爆击（CD 最低约 8 秒） | 高 | 90 | 60 | 保留并调整 target | — | DOTA 7.41 |
| 骷髅王<br>`skeleton_king` | 核心、辅助、承伤、控制、先手 | `hero_damage` | 核心、辅助、承伤、控制、先手；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 550,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 森海飞霞<br>`hoodwink` | 辅助、爆发、机动、控制 | `stun_duration` | 野地奇袭（CD 最低约 12 秒） | 高 | 90 | 70 | 保留并调整 target | — | DOTA 7.41 |
| 森海飞霞<br>`hoodwink` | 辅助、爆发、机动、控制 | `hero_damage` | 辅助、爆发、机动、控制；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 破晓辰星<br>`dawnbreaker` | 核心、承伤 | `healing` | 熠熠生辉；天光现世（CD 最低约 90 秒） | 高 | 100,000 | 60,000 | 保留并调整 target | — | DOTA 7.41 |
| 破晓辰星<br>`dawnbreaker` | 核心、承伤 | `hero_damage` | 核心、承伤；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 玛西<br>`marci` | 辅助、核心、先手、控制、机动 | `stun_duration` | 回身踢（CD 最低约 12 秒） | 高 | 90 | 50 | 保留并调整 target | — | DOTA 7.41 |
| 玛西<br>`marci` | 辅助、核心、先手、控制、机动 | `hero_damage` | 辅助、核心、先手、控制、机动；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 550,000 | 480,000 | 保留并调整 target | — | DOTA 7.41 |
| 獸<br>`primal_beast` | 先手、承伤、控制 | `stun_duration` | 突可击退并眩晕，捶有多次单体眩晕；充分进场时能稳定累计 | 高 | 90 | 100 | 保留并调整 target | — | DOTA 7.41 |
| 獸<br>`primal_beast` | 先手、承伤、控制 | `damage_taken` | 先手、承伤、控制；有明确 Durable 定位和前排能力 | 高 | 600,000 | 240,000 | 保留并调整 target | — | DOTA 7.41 |
| 獸<br>`primal_beast` | 先手、承伤、控制 | `assists` | 已有 `stun_duration` 与 `damage_taken` 两个高匹配指标；为控制每英雄最多两任务，优先删除泛用的助攻指标 | 高 | 100 | — | 删除/替换 | — | DOTA 7.41 |
| 琼英碧灵<br>`muerta` | 核心、爆发、控制 | `hero_damage` | 核心、爆发、控制；兼具核心成长和技能爆发，能通过技能与普攻稳定累计英雄伤害 | 高 | 550,000 | 500,000 | 保留并调整 target | `kills`（建议 40） | DOTA 7.41 |
| 百戏大王<br>`ringmaster` | 辅助、爆发、机动、控制 | `stun_duration` | 实测确认驯兽术的恐惧计入 stun；只按恐惧估值，不假定魅惑也计入 | 高 | 90 | 50 | 保留并调整 target | — | 实测 + DOTA 7.41 |
| 百戏大王<br>`ringmaster` | 辅助、爆发、机动、控制 | `hero_damage` | 辅助、爆发、机动、控制；可依靠范围或爆发技能累计英雄伤害，但辅助定位的 target 应取低档 | 高 | 550,000 | 350,000 | 保留并调整 target | — | DOTA 7.41 |
| 凯<br>`kez` | 核心、机动、控制 | `hero_damage` | 核心、机动、控制；核心定位可依靠装备成长与普攻持续累计英雄伤害 | 高 | 700,000 | 480,000 | 保留并调整 target | `kills`（建议 40） | DOTA 7.41 |
| 朗戈<br>`largo` | 承伤、控制、辅助 | `stun_duration` | 实测确认蛙力千钧的周期眩晕会计入 stun，并可命中多人 | 高 | 90 | 50 | 保留并调整 target | — | 实测 + DOTA 7.41 |
| 朗戈<br>`largo` | 承伤、控制、辅助 | `healing` | 实测确认动人之舐的生命恢复会计入 `GetHealing()` | 高 | 100,000 | 40,000 | 保留并调整 target | — | 实测 + DOTA 7.41 |
...
| metric | 建议 1★ 档位 | 依据 |
|---|---:|---|
| `hero_damage` | 300,000～600,000 | 辅助/前排取低档，高成长 Carry/Nuker 取高档；相较上一版整体适当提高 |
...
`api/src/daily-task/config/tasks.ts` 已按本文建议更新：保留 127 个英雄，英雄任务从 245 条调整为 251 条；每个英雄最多 2 条任务，其中 3 个英雄有 1 条、124 个英雄有 2 条。加上 10 条通用任务后，任务池共 261 条。
...
- `PlayerResource.GetHealing()` 的已知实测口径：兽王斯洛姆战鼓、血魔食血动物、死亡先知驱使恶灵回流治疗不计入；朗戈动人之舐的恢复会计入。当前任务池已无待定的 healing 口径。
