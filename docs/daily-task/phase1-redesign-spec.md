...
每天 3 轮。每轮玩家面对 3 个候选任务（**1 个通用 + 2 个英雄专属**），自由选一个去完成。
...
每轮 3 个候选中固定 **2 个**是英雄专属，且必定是**两个不同英雄**。客户端在判定时检查本局英雄是否匹配 `heroName`，不匹配则该任务不可完成。
...
2. 抽 1 个通用任务
3. 抽第 1 个英雄任务
4. 抽第 2 个英雄任务，**排除与第 1 个相同的 `heroName`**——两个英雄候选必须指向不同英雄，否则这一轮等于只有一个英雄可选
5. 把 `[1, 2, 3]` 三个星级按 seed 洗牌后依次分配给上面三个候选（6 种排列，见 3.3）

**构成为 1 通用 + 2 英雄的理由**：内容量全在英雄侧（307 条 vs 十几条通用），而通用池每天的抽取次数从 6 次降到 3 次，8.1 原先担心的"池子太小、第 3 轮可选面过窄"随之消失，12.2 也不再需要为同一 metric 配多个难度档。代价是不愿换英雄的玩家每轮只剩 1 个实质选项，等于失去 3.3 建立的难度三选一——这是产品取向的取舍，且只是一个常量，上线后看数据可随时调整。
...
- 生成器：同一 `(dayId, steamId, round, completedTaskIds)` 结果稳定；不同 round 结果不同；固定 1 通用 + 2 英雄；同轮两个英雄任务的 `heroName` 不同
