...
| `services/daily-challenge-generation.service.ts` | 134 | **核心算法直接复用**（FNV-1a、seeded pick、`getMetricCategory`）。`pickStar` 改为按 seed 洗牌 `[1,2,3]` 分配（见 3.3）；删掉 `seenTaskIds` 回退后约 110 行 |
