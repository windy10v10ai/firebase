...
- 受范围加成影响的数值后面跟圆圈图标。受技能增强影响的数值不加图标，只按伤害类型上色：星形图标看着像 ※，手机没有悬停说明，玩家看不懂
- 颜色是游戏语义，照游戏显示，与网站「每件事有固定的颜色」的分工不冲突——同 `GameText` 保留本地化里的 `<font color>` 是一个口径
...
|------|------|------|
| 宽度 | 340px | 跟随弹窗 |
| 高度 | 随内容 | 手机：贴底抽屉，随内容、最高到屏幕顶留 3rem；平板起：固定 `min(40rem, 屏高 - 4rem)` |
...
冷却、魔法消耗、范围三个图标是网站自绘的 SVG（14px / 12px），不取游戏素材。魔法消耗图标用 `dota-mana` `#2f7fd6`。
...
- **详情弹窗的付费按钮位置不随技能说明长短变**：手机上是贴底抽屉，按钮落在拇指区；平板起居中且统一高度。弹窗里有付费按钮，位置一跳，想关弹窗的那一下就可能扣了积分
- **说明放不下时直接滚动，平板和手机在渐隐里提示可以滑**：电脑档的滚动条贴着弹窗右边缘，不加提示。触屏的滚动条平时藏着，渐隐底部居中写「滑动查看更多」，旁边的向下箭头轻轻上下动；滑过 8px 提示就消失，这次打开期间滑回顶部也不再出现，关掉重开或换英雄才重新提示。不做「先收起、点『详细』才能滚」：小字链接手指不好点，玩家先去滑又滑不动。文案不写「向下」：写了有人会往下拉，拉在头部会关掉抽屉。头部与付费按钮始终在屏幕内
- **没滚到底时说明区底部都有渐隐**：滚动条自动隐藏的系统上，靠它看出下面还有内容
- **手机抽屉可以从头部往下拖关闭**，顶部有横杠提示：下拉超过 96px 松手关闭，不到则回弹。能拖的是头部整条（左右到抽屉边、往下带上与说明区之间的空隙）加抽屉上沿外 24px 的一段遮罩：单手操作时拇指不容易按准。说明区不参与，不和它的滚动抢手势。右上角 × 仍是主要关闭方式：拖动手势容易被忽略，还可能被系统当成下拉通知栏
- **详情弹窗的关闭是右上角的 ×**，离付费按钮最远；Esc 与点遮罩同样关闭。点遮罩是按下不动就松手，从遮罩上拖不会关闭
- **弹窗打开时背后的页面锁住不滚**：手指在弹窗或遮罩上滑动都不带动页面，全站模态弹窗通用
...
- 占位符取不到值时同步自检报错，不生成
- **本地化文本本身带 KeyValues 转义**（`\n` 换行、`\"` 引号、`\\` 反斜杠），取数时统一反还原；标题、描述最终只走 `GameText` 渲染，只认 `<br>`，所以还原出的换行要接着转成 `<br>`。产物里不允许残留任何反斜杠转义，同步自检拦这条
