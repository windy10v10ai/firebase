...
- **说明放不下时，电脑档直接滚动，平板和手机先收起**：电脑有滚轮，多点一次「详细」反而麻烦，滚动条贴着弹窗右边缘；触屏第一眼不出滚动条，底部渐隐加「详细」链接，点了才能滚动，并自动滚到被渐隐盖住的地方。头部与付费按钮始终在屏幕内
- **没滚到底时说明区底部都有渐隐**：滚动条自动隐藏的系统上，靠它看出下面还有内容
- **手机抽屉可以从头部往下拖关闭**，顶部有横杠提示：下拉超过 96px 松手关闭，不到则回弹。能拖的是头部整条（左右到抽屉边、往下带上与说明区之间的空隙）加抽屉上沿外 24px 的一段遮罩：单手操作时拇指不容易按准。说明区不参与，不和它展开后的滚动抢手势。右上角 × 仍是主要关闭方式：拖动手势容易被忽略，还可能被系统当成下拉通知栏
