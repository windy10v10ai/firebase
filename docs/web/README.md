...
Dota 2 7.41f 起，从游廊直接开局的对局发不出任何网络请求，网站成了改玩家数据的唯一入口。玩家要让游戏读到最新数据，只能带启动参数开局：`/offline` 页上的 `steam://run/570//%2Bdota_launch_custom_game%20<id>%20<map>/` 链接由 Steam 代为拉起 Dota 并带上参数，玩家不用开控制台、不用敲命令。

**命名按三个维度分开，不合成一个词**，取值见[根目录 CLAUDE.md](../../CLAUDE.md) 的「用语」：玩家怎么开始（从游廊开局 / 在网站上开始游戏）、这局连没连服务器（离线 / 在线）、这局跑在谁的机器上（本地主机 / 服务器主机）。

之所以不拿「在线 / 离线」当动作名，是因为将来会多一种由服务器主机开的局，它同样从网站上开始、同样是在线。动作名按「在哪开始」命名，到那时只要在 `/offline` 的对比表里加一行主机、把在线列拆成两列，入口文案一个字都不用改；按「连不连服务器」命名则要全站重写。

对文案的约束：

- **说生效时机必须带上开局方式。** 从游廊开局等下次地图更新，在网站上开始游戏下一局生效、游戏内点「刷新玩家信息」当场生效。不分方式的「立即生效」「回游戏点刷新」都是错的。
- **入口说动作，不上状态词。** 玩家心里的话是「我改了怎么游戏里没变」，所以页脚、首页卡、提示条写怎么开局、什么时候在游戏内生效；哪种方式是离线、哪种是在线，只在 `/offline` 解释一次。
...
- **控制台命令是兜底，不是主路径。** 浏览器拦下链接、或 Steam 没接管这类链接时才用，收在 `/offline` 底部的折叠块里。

`/offline` 页按「先给操作、再讲差别、最后兜底」排：启动链接与两个系统提示的说明在最上面，两种方式的对比表在中间，控制台命令折叠在最后。玩家进这一页多半是想开局，不是想读说明。启动前必须完全退出 Dota 2——游戏开着时点链接毫无反应，这句提示比链接本身还靠前。
...
- 两种开局方式的完整规则只在 `/offline` 一处讲，其余页面放一句提示加链接。分散在各页各写一套，改一次口径要追七八处，迟早互相打架。
