...
- 每日任务的 30 天历史只有网站能看全，游戏内那份最多 5 天：客户端代发通道把数据塞在 HTML 的 `<title>` 里，超过 4096 字符会被静默截断。历史是后端已经在存的现成数据，与上一条「不做记录页」不冲突——那条说的是积分流水，要新建集合并在每处积分变动加写入。做法见 [phase-14-daily-task.md](../design/web/phase-14-daily-task.md)。
