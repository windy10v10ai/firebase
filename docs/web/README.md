...
中文、英文、俄语三种，做法与取舍见 [phase-15-russian-locale.md](../design/web/phase-15-russian-locale.md)。
...
**没译到的 key 回落英文，不报错。** 一种语言可以只译一部分就上线，缺的位置出英文字而不是空白或 key 名。等整份译完再上线会让第一版遥遥无期，混排的英文至少能读。
...
- 界面语言支持中、英、俄三种，选择存 cookie 不进 URL；没译到的 key 回落英文，一种语言可以只译一部分就上线。做法见第 4 节「多语言」。
