...
- **平板横排只放四项**：启动游戏、个人主页、任务、会员。属性与觉醒到电脑档才出现，`config/nav.ts` 用 `desktopOnly` 标出；这一档放不下六项，俄语尤其放不下，理由见第 4 节「多语言」。
...
**俄语文案比中英都长，横排导航因此只在电脑档放满。** 768 一档最多放得下四项，俄语六项要 916px。让位的项（属性、觉醒）在 `config/nav.ts` 标 `desktopOnly`，菜单里照常出现。
