...
Dota 2 7.41f 起，游廊里开的对局服务端发不出网络请求，改由玩家的游戏客户端代发（做法见 game 仓库的 `src/vscripts/api/README.md`）。所以从游廊开局平时就是在线的，能读最新数据、正常结算，也能多人。代发偶尔会失败，这局就退回地图更新时导出的数据，也就是离线。
...
`/launch` 页按「先讲差别、再给备用入口、最后兜底」排：两种方式的对比在最上面，先告诉玩家平时从游廊开局就行；网站启动的链接与两个系统提示的说明在中间；控制台命令在最后。启动前必须完全退出 Dota 2——游戏开着时点链接毫无反应，这句提示紧挨在链接前面。首页的启动卡片排在功能卡片之后。
