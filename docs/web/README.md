...
| `/offline` | 离线模式与在线模式说明 | 否 | 页脚、首页的说明卡、属性与觉醒页的提示条 |
...
- **不承诺游戏内立即生效。** 离线模式下网站上的改动要等下次地图更新，在线模式下是下一局。哪个都不是「点一下就同步」，任何「回游戏点刷新」「立即生效」的引导都是错的。
- **不引导玩家在游戏内操作账号数据。** 加点、洗点、觉醒、消费会员积分在离线模式下写不回服务器，文案一律指向网站。
- **完整规则只在 `/offline` 讲一遍。** 其余页面只放一句提示加一条指向它的链接，不各写一套；玩家在哪一页看到的说法都必须对得上。
- **不说「打一局就有数据」。** 结算、战绩、行为分只在在线模式记录，离线模式的对局不回传。
