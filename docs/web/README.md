...
| `/offline` | 启动方式与数据生效时机 | 否 | 头部菜单「数据生效时机」（只进汉堡菜单）、页脚、首页的说明卡、属性与觉醒页的提示条 |
...
**只进菜单、不进横排的项标 `menuOnly: true`。** 768 那一档横排已经被四个功能项加 GitHub、语言、账号、汉堡排满，再加文字项会溢出。说明类页面（`/offline`）不是功能入口，放汉堡菜单里就够。

### 两种开局模式

Dota 2 7.41f 起，从游廊直接开局的对局发不出任何网络请求，网站成了改玩家数据的唯一入口。玩家因此有两种开局方式，网站文案统一叫**离线模式**与**在线模式**，两个名字和 game 侧一致，取值见[根目录 CLAUDE.md](../../CLAUDE.md) 的「用语」。

对文案的约束：

- **说生效时机必须带上模式。** 离线模式等下次地图更新，在线模式下一局生效、游戏内点「刷新玩家信息」当场生效。不分模式的「立即生效」「回游戏点刷新」都是错的。
- **不引导玩家在游戏内操作账号数据。** 为了保护玩家数据安全，加点、洗点、觉醒只在网站上做，两种模式都一样，游戏内那两个按钮是跳转到网站。
- **入口说现象，不上术语。** 玩家心里的话是「我改了怎么游戏里没变」，所以页脚、首页卡、提示条写「什么时候在游戏内生效」，两个模式名只在 `/offline` 解释一次。
- **完整规则只在 `/offline` 讲一遍。** 其余页面只放一句提示加一条指向它的链接，不各写一套；玩家在哪一页看到的说法都必须对得上。
- **不说「打一局就有数据」。** 结算积分、每日任务、战绩、行为分、每日签到与活动积分都只在在线模式记录，离线模式的对局不回传。
...
- 离线模式与在线模式的完整规则只在 `/offline` 一处讲，其余页面放一句提示加链接。分散在各页各写一套，改一次口径要追七八处，迟早互相打架。
