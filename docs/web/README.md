...
| 功能入口 | 属性卡跳属性页、标签显示可用属性点；觉醒卡跳觉醒页、标签显示已觉醒 / 可觉醒总数；每日任务卡跳每日任务页，不带标签——轮次要多发一次请求，而那条接口会写库 |
...
电脑宽度分成两栏（1 : 2）：左栏身份卡，右栏等级卡与两张入口卡，两张战绩卡通栏。**会员的东西集中在身份卡**：会员状态、有效期、签到、订阅入口是同一件事的四个面，分散在两张卡里玩家要来回找。**可用属性点只在入口卡上出现一次**：等级卡里再放一行，与正下方的入口卡同数字、同去向。**会员状态并进身份卡，不单独成卡**：单独一张卡只有两行字加一个按钮，占着左栏、下半截空着；订阅入口用文字链接，会员状态在这里只做陈述，不需要与页面主操作同等分量的金色按钮。
