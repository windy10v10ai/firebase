...
- **动作名只用「从游廊开局」「在网站上开始游戏」**，不拿连接状态当动作名，不写「用在线模式开局」
- **「离线」「在线」只说连接状态**，用在解释数据新旧与 `/offline` 的对比表里；「本地主机」「服务器主机」只说主机，不指代连接状态
- **入口不上状态词**：页脚、首页卡、提示条说怎么开局、什么时候在游戏内生效，哪种方式是离线只在 `/offline` 解释
- **说生效时机必须带上开局方式**：从游廊开局等下次地图更新，在网站上开始游戏下一局生效、游戏内刷新当场生效。不写不分方式的「立即生效」「回游戏点刷新」，也不写「打一局就有数据」
- **完整规则只写在 `/offline`**，别的页面挂 `OfflineNotice` 或一条指向它的链接，不重写一遍
- **控制台命令只出现在 `/offline` 底部的折叠兜底块里**，不当主路径写
