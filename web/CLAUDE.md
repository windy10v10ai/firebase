...
- **生效时机只说「下一局生效」**，不分开局方式；离线局沿用地图更新时的数据，只在 `/launch` 对比表下说明。不写「立即生效」「回游戏点刷新」
- **完整规则只写在 `/launch`**，别的页面不重写开局方式与生效时机
- **不向玩家解释连不上的原因，也不列游戏内暂缺的功能**，只说游廊开局「偶尔可能连不上」
...
改了页面外观，PR 正文里要放改动后的截图。

**默认只拍改动后，不拍改动前。** 读者要看的是页面现在长什么样，改动前的样子线上和历史 PR 里都查得到，每次都起基线拍一遍成本翻倍。只有两种情况放前后对比：用户明确要求；或者这个改动的价值就在细节差异上，比如微调间距、字号、配色，或声称外观不变的依赖升级、构建器替换、公共组件重构，不摆在一起看不出变没变。
...
**做前后对比且没有差异时**：无头 Chrome 用同一组参数拍出来的图，页面没变就是逐字节相同的 PNG。`md5 -q` 比一下，相同就只贴一份图，把 md5 写进正文说明另一份一样——这比自算的哈希更有说服力，因为读者可以自己重拍一张对。
