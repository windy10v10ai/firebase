...
export default function IdentityCard({ info }: { info: PlayerInfo }) {
  ...
  return (
    <section className="card-container @container space-y-4 p-6 sm:p-8">
      <h1 className="title-primary">{t('heading', { id: info.id })}</h1>
      <StatList
        items={[
          { label: t('battleLevel'), value: String(info.seasonLevel), tone: 'season' },
          { label: t('memberLevel'), value: String(info.memberLevel), tone: 'member' },
          {
            label: t('battlePoint'),
