...
export default function SampleDataPanel() {
  ...
  // 窄屏只有两列，放下六格就满了，收起的六格挑的是重复度最高的那些
  const narrowHidden = 'hidden sm:block';
  ...
  return (
    ...
    >
      ...
      <div className={narrowHidden}>
        <Tile label={t('vision')}>
          <Value className="text-heading">
            +100
            <span className="ml-1.5 text-[11px] font-normal text-muted">{t('level', { n: 2 })}</span>
          </Value>
        </Tile>
      </div>
      <Tile label={t('cooldown')}>
        <Value className="text-heading">
          −12%
          <span className="ml-1.5 text-[11px] font-normal text-muted">{t('level', { n: 3 })}</span>
        </Value>
      </Tile>
