...
**长期有效的内容随改动同步，不等用户提醒。**一次设计或实现如果新增或改变了某模块长期有效的决定（架构、鉴权、技术选型、全站通用的布局与视觉约定），在同一个 PR 里更新对应的 `docs/<模块>/` 框架性文档；只写进批次设计文档不算完成，批次文档会被瘦身甚至删除，长期决定留在那里会跟着消失。**规范本身（取值、规格、选型规则）也要进框架性文档，不是只在 README 写几句结论、把规范留在批次文档。**文档配图只放实现后的最终截图，方案对比等过程图不进 git。
