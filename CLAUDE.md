...
| 模式 | 中文文案 | 英文文案 | 玩家看到什么 |
|---|---|---|---|
| 从游廊直接开局 | 离线模式 | Offline mode | 多人联机；会员、属性、觉醒沿用地图更新时导出的数据；本局积分不记录，也看不到每日任务；网站上的改动下次地图更新后生效 |
| 用控制台命令开局 | 在线模式 | Online mode | 只能单人；积分与每日任务正常记录；网站上的改动下一局即生效 |

「控制台启动」只用来说明怎么进入在线模式，不当作模式名。不用「离线数据」「本地主机」「实时数据」指代模式。完整说明与理由见 [docs/web/README.md](docs/web/README.md) 的「两种开局模式」。
