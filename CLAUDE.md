...
| 开局方式 | 连接状态 | 主机 | 玩家看到什么 |
|---|---|---|---|
| 从游廊开局 | 在线，连不上服务器时离线 | 本地主机 | 平时用的方式，多人联机；读最新数据，结算积分、每日任务、每日签到与活动积分都正常，游戏内可以「刷新玩家信息」；偶尔连不上服务器，这局就是离线：会员、属性、觉醒沿用地图更新时导出的数据，本局积分不记录 |
| 从网站启动游戏 | 在线 | 本地主机 | 备用方式，游廊开局连不上服务器时用；只能单人，连接更稳 |

- **动作名只用「从游廊开局」「从网站启动游戏」**，不拿连接状态当动作名，不写「用在线模式开局」。「用控制台启动」降级成兜底说明，只出现在 `/launch` 最后一节
- **「离线」「在线」是连接状态的正式说法**，游戏内显示连接状态的地方照常用；网站上用在解释数据新旧与两种方式的对比
...
- **网站启动是备用入口，降级不删除**：将来服务器主机要从网站开局，入口一直留着，玩家不会觉得它时有时无
