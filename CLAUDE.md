...
| 维度 | 中文文案 | 英文文案 |
|---|---|---|
| 玩家怎么开始 | 从游廊开局 / 在网站上开始游戏 | Launched from the Arcade / Started from this site |
...
| 在网站上开始游戏 | 在线 | 本地主机 | 只能单人；结算积分、每日任务、每日签到与活动积分都正常；游戏内可以「刷新玩家信息」；网站上的改动下一局即生效 |

- **动作名只用「从游廊开局」「在网站上开始游戏」**，不拿连接状态当动作名，不写「用在线模式开局」。「用控制台开局」降级成兜底说明，只出现在 `/offline` 的折叠块里
...
**入口说动作，不上状态词**：页脚、首页卡、提示条写怎么开局、什么时候在游戏内生效，「哪种方式是离线、哪种是在线」只在 `/offline` 页解释一次。完整说明与理由见 [docs/web/README.md](docs/web/README.md) 的「开局方式与命名」。
