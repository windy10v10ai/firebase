...
玩家有两种开局方式，文案里只用这两个名字，与 game 侧一致：

| 模式 | 中文文案 | 英文文案 | 玩家看到什么 |
|---|---|---|---|
| 从游廊直接开局 | 离线模式 | Offline mode | 多人联机；会员、属性、觉醒沿用地图更新时导出的数据；结算积分与每日任务不记录，每日签到和活动积分也领不到；网站上的改动下次地图更新后生效 |
| 用控制台命令开局 | 在线模式 | Online mode | 只能单人；结算积分、每日任务、每日签到与活动积分都正常；游戏内可以「刷新玩家信息」；网站上的改动下一局即生效 |

「控制台启动」只用来说明怎么进入在线模式，不当作模式名。不用「离线数据」「本地主机」「实时数据」指代模式。加点与觉醒两种模式都只能在网站上做，游戏内那两个按钮是跳转到网站。

**入口说现象，不上术语**：页脚、首页卡、提示条写「什么时候在游戏内生效」，两个模式名只在 `/offline` 页解释一次。完整说明与理由见 [docs/web/README.md](docs/web/README.md) 的「两种开局模式」。
